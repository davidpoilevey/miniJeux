import WorldGrid, { GRID_COLS, GRID_ROWS } from './WorldGrid';
...
export function getTerrainType(altitude, humidity, temperature, seaLevel = DEFAULT_SEA_LEVEL) {
  ...
  if (altitude >= 50 && temperature < 3) return 'neige';
  if (altitude >= 65)            return 'montagne';
...
function buildAltitudeMap(cols, rows, seed) {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      // Mélange — continental fort → dorsales montageuses cohérentes
      const raw = continental * 0.40 + local * 0.35 + island * 0.25;

      // Redistribution : exposant 1.5 (2 écrasait trop les hautes valeurs → zéro montagne)
...
export default function generateWorld(cols = GRID_COLS, rows = GRID_ROWS, seed = 42, seaLevel = DEFAULT_SEA_LEVEL) {
